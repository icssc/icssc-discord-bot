...
use sea_orm::{ColumnTrait, Condition, EntityTrait as _, QueryFilter, QueryOrder as _};
...
pub(crate) async fn history(
    ctx: AppContext<'_>,
    #[description = "Type of spotting to filter by"] 
    mut r#type: Option<HistoryFilterType>,
    #[description = "User who is present in either a snipe or social"]
    participant: Option<User>,
    #[description = "(snipes only) the person taking the snipe"] 
    sniper: Option<User>,
    #[description = "(snipes only) the person being sniped"] 
    snipe_victim: Option<User>,
) -> Result<(), AppError> {
    let has_snipe_field = sniper.is_some() || snipe_victim.is_some();

    if has_snipe_field {
        ensure!(participant.is_none(), "`participant` cannot be used with `sniper` or `snipe_victim`");
        // automatically set the filter type if possible
        r#type = match r#type {
            Some(HistoryFilterType::Social) => bail!("`sniper` or `snipe_victim` is for snipes only"),
    ...
    .into_iter()
    .flatten()
    .fold(Condition::all(), |acc, curr| acc.add(curr));
